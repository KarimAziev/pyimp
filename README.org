...
Project modules are discovered through Emacs's =project.el= integration.  This
means version-control ignores such as =build/=, =dist/=, and virtual
environments are respected, while new untracked source files remain available.
Pyimp also recognizes ignored generated modules inside regular Python packages
(directories containing =__init__.py=) without treating arbitrary ignored build
trees as import roots.
...
-  =pyimp-project-import-roots=
   Adds Python import roots that cannot be inferred automatically.  Relative
   directories are resolved from the Emacs project root; absolute directories
   are used as written.  Pyimp automatically detects the current package root
   and the conventional =src/= layout, so this option is mainly useful for
   monorepos or projects with multiple source trees.

   For example:

#+begin_src elisp
((python-ts-mode . ((pyimp-project-import-roots . ("backend/src"
                                                    "shared/python")))))
#+end_src
